    ...
    for list in linestring:
      ...
      for tuple in match:
        ...
        if len(tuplelist) != 1:
          ...
          weight0 = dist_sphere.distance_between_points(tuplelist[0][1][1],tuplelist[0][1][0],tuplelist[1][1][1],tuplelist[1][1][0])*20.0
          ...
          for index in range(1,len(tuplelist)-1):
            ...
            weight1 = dist_sphere.distance_between_points(tuplelist[index][1][1],tuplelist[index][1][0],tuplelist[index+1][1][1],tuplelist[index+1][1][0])*20.0
            weight2 = dist_sphere.distance_between_points(tuplelist[index][1][1],tuplelist[index][1][0],tuplelist[index-1][1][1],tuplelist[index-1][1][0])*20.0
            graphdict[key][value1] = weight1
            graphdict[key][value2] = weight2

          keyn = tuplelist[-1][0]
          valuen = tuplelist[-2][0]
          weightn = dist_sphere.distance_between_points(tuplelist[-1][1][1],tuplelist[-1][1][0],tuplelist[-2][1][1],tuplelist[-2][1][0])*20.0
